...
class Quad:
    ...
    def progress_segment(self,
                         ...
                         end_co: mathutils.Vector) -> Tuple[List[bmesh.types.BMEdge], bmesh.types.BMVert]:
        selected_edges: List[bmesh.types.BMEdge] = []
        for edge in vert.link_edges:
            selected_edges = [edge]
            now_edge: bmesh.types.BMEdge = edge
            next_vert = now_edge.other_vert(vert)
            for _ in range(nedges - 1):
                ...
                now_edge = next_edge
                next_vert = now_edge.other_vert(next_vert)
    ...
    def select_quad_edge(self, edge: List[Segment], nedges: int, glist: "GlobalList", bm: bmesh.types.BMesh):
        selected_edges: List[bmesh.types.BMEdge] = []
        vert = glist.search_vert(bm, edge[0].p1)
        for segment in edge:
            res = self.progress_segment(nedges, vert, glist.get_coords(segment.p2.i))
...
class GlobalList:
    ...
    def search_vert(self, bm: bmesh.types.BMesh, point: Point) -> bmesh.types.BMVert:
        for v in bm.verts:
            if same_coords(v.co, self.reduced_points[point.i]):
                return v
